...
required: false
  ...
  run: async (client, interaction, args) => {
    ...
    var interactionA = await interaction.followUp({ content: `<@${interaction.member.user.id}>`, embeds: [new MessageEmbed().setColor("WHITE").setDescription("<a:loading_:906786750494564353> Please wait...").setFooter(interaction.member.user.tag).setThumbnail(interaction.member.user.displayAvatarURL())] });
    ...
    if (checkIfExsists === null) {
      ...
      await UrlsConfig.create({
        ...
        await interactionA.edit({ embeds: [new MessageEmbed().setTitle("<:Check:906789001422327858> Added Succesfully").setColor("RED").setDescription("Thanks for using me").setTimestamp()] });
